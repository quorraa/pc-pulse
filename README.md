...
1. Download `PcPulse-1.5.3-x64.msi` and `SHA256SUMS.txt` from the [latest release](https://github.com/quorraa/pc-pulse/releases/latest).
...
```powershell
.\scripts\Build-Release.ps1 -Architecture x64 -Version 1.5.3
```

For a signed release:

```powershell
.\scripts\Build-Release.ps1 -Version 1.5.3 -CertificateThumbprint YOUR_SHA1_THUMBPRINT
```

Outputs are placed in `artifacts`, including `PcPulse-1.5.3-x64.msi`, `SHA256SUMS.txt`, and:
...
```powershell
msiexec.exe /i .\artifacts\PcPulse-1.5.3-x64.msi
```
...
```powershell
msiexec.exe /x .\artifacts\PcPulse-1.5.3-x64.msi
```
