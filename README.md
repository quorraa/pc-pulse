...
1. Download `PcPulse-1.8.0-x64.msi` and `SHA256SUMS.txt` from the [latest release](https://github.com/quorraa/pc-pulse/releases/latest).
...
```powershell
.\scripts\Build-Release.ps1 -Architecture x64 -Version 1.8.0
```
...
```powershell
msiexec.exe /i .\artifacts\PcPulse-1.8.0-x64.msi
```
...
```powershell
msiexec.exe /x .\artifacts\PcPulse-1.8.0-x64.msi
```
