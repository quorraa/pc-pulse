...
Five CLIENT rows on the Settings page control it: "Background video" (the path; Enter edits, an empty value turns it off), "Background quality" (Enter cycles low 208x116 → medium 416x232 → high 832x464 → ultra 1248x702; each quality is converted once, so changing it converts the video again at the new size), "Background" (on/off), "Background dim" (10-60%, how far the clip is darkened toward the theme), and "Background fps" (auto follows the clip's own rate, or 1-60 — the frame-budget guardrail lowers the background's rate before it ever touches UI refresh, shown as e.g. `60 → 30 (auto)`). The background needs at least a 72x20 terminal; below that it stays hidden.
