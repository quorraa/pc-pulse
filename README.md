...
1. Download `PcPulse-1.18.0-x64.msi` and `SHA256SUMS.txt` from the [latest release](https://github.com/quorraa/pc-pulse/releases/latest).
...
Every finding is measured against a per-machine baseline the collector learns for itself, not a fixed number pulled from a different computer. For the first 24 hours on a machine it stays quiet by default — only a high-confidence Critical issue breaks through — while everything it sees still lands in history so nothing is lost while it learns; the TUI's statusline says so plainly while it's happening. Once baselined, a condition has to cross its threshold and stay there, with hysteresis on the way back down, before it becomes a finding at all, and a finding that keeps recurring reopens the same incident instead of spawning a new one every time it refires — one growing history with an occurrence count, not a wall of near-duplicates. Notification itself is gated on more than "did it cross the line": confidence, how long it has persisted, whether other signals corroborate it, and whether it is actually affecting you. A condition that is real but thin on evidence gets recorded, not popped in your tray — you can always go look.

The same calibration applies to the collector's watch over itself: its CPU, memory, and handle budgets carry a tolerance band, so anything up to 15% over the ceiling is recorded quietly rather than escalated straight to Critical. DPC and interrupt attribution only pops a balloon on repeatable evidence — the same driver family named across multiple captures, or a sustained rate above your machine's own learned normal corroborated by other activity — so one noisy capture or attribution that flips between guesses stays in the evidence trail instead of the notification queue. Handle and thread growth gets the same discrimination: a burst that climbs and then plateaus or gives ground back is a shape the collector recognizes and resolves silently, while a genuine monotonic climb over time still gets flagged.
...
```powershell
.\scripts\Build-Release.ps1 -Architecture x64 -Version 1.18.0
```
...
```powershell
msiexec.exe /i .\artifacts\PcPulse-1.18.0-x64.msi
```
...
```powershell
msiexec.exe /x .\artifacts\PcPulse-1.18.0-x64.msi
```
